// components/AntimicrobialForm.js
import React, { useState } from 'react';
import axios from 'axios';

function AntimicrobialForm({ patientId, token }) {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    await axios.post(
      'http://https://hospital-management-backend-mqyh.onrender.com/api/patients/antimicrobial-record',
      { ...form, patient_id: patientId },
      { headers: { Authorization: `Bearer ${token}` } }
    );
    alert('Antimicrobial record saved!');
  };

  return (
<form onSubmit={handleSubmit} className="space-y-4 bg-white shadow-md rounded p-6 mt-4 max-w-xl">
  <h3 className="text-lg font-semibold text-gray-800">Antimicrobial Record</h3>
